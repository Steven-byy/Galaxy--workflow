...
MAX_IMG_DEFAULT = [30000, 30000, 30000, 30000, 30000]


class GalaxyDataset():
    def __init__(self, max_img=MAX_IMG_DEFAULT, seed=10, metadata="training_solutions_rev1.csv",
                 available_images_log="full_galaxy_data.log"):
        if len(max_img) < 5:
    ...
    def label_dataset(self):
        final_table_columns = ['GalaxyID', 'Class1.1', 'Class1.2', 'Class2.1', 'Class2.2', 'Class4.1', 'Class7.1',
                               'Class7.2', 'Class7.3']
        ...
        df['Label'] = np.where((df['Label'] == '$') & (df['Class1.1'] >= 0.469) & (df['Class7.2'] >= 0.500), '1',
                               df['Label'])
        df['Label'] = np.where((df['Label'] == '$') & (df['Class1.1'] >= 0.469) & (df['Class7.3'] >= 0.500), '2',
                               df['Label'])
        df['Label'] = np.where((df['Label'] == '$') & (df['Class1.2'] >= 0.430) & (df['Class2.1'] >= 0.602), '3',
                               df['Label'])
        df['Label'] = np.where(
...
def main():
    ...
    parser.add_argument("-m", "--max_img", metavar=("MAX_IMG_0", "MAX_IMG_1", "MAX_IMG_2", "MAX_IMG_3", "MAX_IMG_4"),
                        type=int, nargs=5, default=MAX_IMG_DEFAULT, help="number of instances of each class")
    parser.add_argument("-s", "--seed", type=int, default=10, help="seed for random library")
    parser.add_argument("-d", "--dataset-metadata", type=str,
                        default="../galaxy-zoo-the-galaxy-challenge/training_solutions_rev1.csv",
                        help="Dataset metadata")
    parser.add_argument("-g", "--galaxy-log-file", type=str, default="../config/full_galaxy_data.log",
                        help="Available galaxy images")
    ...
    for files in os.listdir(dir):
        if files in list0:
            newname = list1[list0.index(files)]
            os.rename("../full_galaxy_data/" + files, "../full_galaxy_data/" + newname)
        else:
            os.remove("../full_galaxy_data/" + files)
